...
class SchreierContext:
    ...
    def _initial_partition(
        ...
        if graph_coloring is None:
            ...
            num_colors = max(graph_coloring.values()) + 1
            partition = [set() for _ in range(num_colors)] + [None] * (self._num_nodes - num_colors)
            for node_label, node_color in graph_coloring.items():
                node_index = self.node_to_index[node_label] # TODO ensure this works with colors not starting at 0..n-1
                partition[node_color].add(node_index)
            ...
            #num_colors = i + 1
            #partition = partition + [None] * (self._num_nodes - num_colors)
...
def schreier_rep(
    ...
    if nx.number_connected_components(graph) == 1:
        ...
        initial_partition, trace, color, num_colors = ctx._initial_partition(graph_coloring)
