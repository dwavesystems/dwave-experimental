...
class ShimmingMockSampler(MockDWaveSampler):
    ...
    def __init__(
            self, topology_type="zephyr", topology_shape=[3], flux_biases_baseline=None,
            use_SA=False
    ):
        if use_SA:
...
class FluxBiases(unittest.TestCase):
    ...
    def test_flux_params(self):
        ...
        nv = 10
        bqm = dimod.BinaryQuadraticModel('SPIN').from_ising({i: 1 for i in range(nv)}, {})
        ...
        sampling_params = {'num_reads': 1, 'flux_biases': [val]*sampler.properties['num_qubits']}
        fb, fbh, mh = shim_flux_biases(bqm, sampler, sampling_params=sampling_params, learning_schedule=[]) # , shimmed_variables, learning_schedule, convergence_test, symmetrize_experiments
        self.assertTrue(all(x==y for x,y in zip(fb,sampling_params['flux_biases'])))
        self.assertTrue(all(x==val for x in fb))
        # No movement if converged:
        fb, fbh, mh = shim_flux_biases(bqm, sampler, sampling_params=sampling_params, convergence_test = lambda x,y: True)
        self.assertTrue(all(x==y for x,y in zip(fb,sampling_params['flux_biases'])))
        self.assertTrue(all(x==val for x in fb))
