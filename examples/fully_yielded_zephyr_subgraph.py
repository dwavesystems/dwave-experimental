...
def main(family="zephyr", seed=None, m_t=12, m_s=5, t=4, t_s=2, node_yield=0.97):
    ...
    num_remove = int((1-node_yield) * (target.number_of_nodes() - len(protected_nodes)))
    ...
    G = target.copy()
    G.add_nodes_from(removed_nodes) # For sake of plotting we add back singleton nodes (no edges)
    node_color = ['r' if G.degree(n)==0 else 'lightgray' for n in G.nodes()]
    plt.figure('Successful search by displacement')
    dnx.draw_parallel_embeddings(G=G, embeddings=tile_embeddings, S=tile, one_to_iterable=False, shuffle_colormap=False, node_color=node_color)
    ...
    removed_nodes2 = [tuple(n) for n in rng.choice(list(protected_nodes), size=num_remove, replace=False)]
    ...
    if source.graph["family"] == "pegasus":
        ...
        # generators.
        source.remove_nodes_from([n for n in source.nodes() if n[2] % 2 != 0]) # remove odd k nodes to get single-rail pegasus graph
    print(
        "Step 6: With any defects present on a sublattice, the graph of shape " \
        f"{shape_s} will not embed. "
        f"However a graph of shape {shape_step6} with {source.number_of_nodes()} nodes and "
        f"{source.number_of_edges()} edges may be feasible."
    )
    ...
    if metadata.final_num_yielded < metadata.max_num_yielded:
        ...
        fig_name = 'Successful graph-minor search by quotient search + minorminer refinement'
    else:
        print(
            "Step 8: The quotient search already achieved full yield, so no refinement step is needed."
        )
        node_color = ['r' if target_sub.degree(n)==0 else 'lightgray' for n in target_sub.nodes()]
        fig_name = 'Successful subgraph automorphism search by quotient search'
    plt.figure(fig_name)
    plt.title(fig_name)
    G = target_sub.copy()
    G.add_nodes_from(inv_map[n] for n in removed_nodes2) # For sake of plotting we add back singleton nodes (no edges)
    node_color = ['r' if G.degree(n)==0 else 'lightgray' for n in G.nodes()]
    dnx.draw_parallel_embeddings(G=G, embeddings=[emb], S=source, one_to_iterable=True, shuffle_colormap=False, node_color=node_color)
